...
function ProductsContent({ initialProducts = [] }) {
  ...
  const handleMenuClick = (id, e) => {
    ...
    if (openMenuId === id) {
      ...
      // Calculate top position (open downwards if space, else upwards)
      let top = rect.bottom + 4;
      if (top + menuHeight > window.innerHeight && rect.top - menuHeight > 0) {
        top = rect.top - menuHeight - 4;
      }
      
      // Calculate left position (prefer aligning to trigger left, keep within screen)
      let left = rect.left;
        ...
        if (data.products && Array.isArray(data.products)) {
          const mapped = data.products.map(p => {
            ...
            return {
              ...
              style: p.style,
              lengths: p.lengths || [],
              sizes: isFactory ? [] : (Array.isArray(p.sizes) && p.sizes.length > 0 ? p.sizes : ["S", "M", "L"]),
              color: p.color ? p.color.split(',')[0].trim() : null,
              colors: p.color ? p.color.split(',').map(c => c.trim()).filter(Boolean) : [],
              images: p.images || [],
